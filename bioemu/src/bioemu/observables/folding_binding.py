...
class FoldingBinding:
    ...
    def __call__(self, batch: ChemGraph, sequence: str) -> torch.Tensor:
        """
        Compute folding and binding free energy for a batch of ChemGraph samples.
        ...
        h = torch.zeros((coords.shape[0], 2), device=coords.device)
        ...
        # Compute binding-esque free energy
        # from definition in the GRB2_SH3 mutants csv
        residues_interface = [6, 8, 11, 12, 15, 31, 33, 34, 36, 45, 47, 49, 50]
        p_bound = compute_folded_proportion(coords[:, residues_interface], ref_coords[residues_interface], self.k, self.d_0, self.tol)
